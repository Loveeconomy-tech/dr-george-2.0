...
const NavbarLink: React.FC<{ item: ILink; transparent: boolean }> = ({
  item,
  transparent,
}: {
  item: ILink;
  ...
  return (
    <>
      {item.type && item.type === "button" ? (
        <Link as={NextLink} href={item.path} passHref>
          <CustomButton
  ...
  return (
    <Box display={{ base: "none", md: "flex" }}>
      ...
      >
        ...
        <Flex color={"white"} gap={8}>
          <Box borderBottomWidth={2} borderColor={"base.blue"} px={1}>
          ...
          <Menu>
            ...
            >
              {language !== "en" ? (
                <MenuItem w={"100%"} onClick={() => changeLanguage("en")}>
                  EN
                </MenuItem>
              ) : null}
              {language !== "fr" ? (
                <MenuItem w={"100%"} onClick={() => changeLanguage("fr")}>
                  FR
                </MenuItem>
              ) : null}
              {language !== "es" ? (
                <MenuItem w={"100%"} onClick={() => changeLanguage("es")}>
                  ES
                </MenuItem>
              ) : null}
              {language !== "pt" ? (
                <MenuItem w={"100%"} onClick={() => changeLanguage("pt")}>
                  PT
                </MenuItem>
              ) : null}
              {language !== "ch" ? (
                <MenuItem w={"100%"} onClick={() => changeLanguage("ch")}>
                  CH
                </MenuItem>
              ) : null}
            </MenuList>
          ...
          >
            <Flex w={24} justify={"center"} alignItems={"center"} p={1}>
              {transparent ? (
                <Image
                  className="logo-box"
                  src="/images/G W Arthur Logo-white.png"
                  alt="gwarthur logo"
                />
              ) : (
                <Image
                  className="logo-box"
                  objectFit={"contain"}
                  src="/images/G W Arthur Logo.png"
                  alt="gwarthur logo"
                />
              )}
            </Flex>
